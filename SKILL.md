...
description: Navigate Qunar (去哪儿) for flight deals, hotel bookings, travel optimization strategies, and browser-based live fare/hotel checks.
metadata:
  clawdbot:
    emoji: "✈️"
    requires:
...
When the task requires live Qunar page inspection, follow the shared **browser-commerce-base** workflow:
- public search/detail pages → `openclaw`
- order/account/member pages → `user` only when needed
- re-snapshot after date, filter, room, or fare changes
- capture final price, refund policy, and hidden-fee risk in evidence screenshots

Key browser extraction order on Qunar:
- 航班/酒店标题
- 当前价 / 到手价
- 舱位 / 房型 / 套餐差异
- 退改政策与附加费用
